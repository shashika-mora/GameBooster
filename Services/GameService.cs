...
public sealed class GameService : IDisposable
{
    ...
    public void Launch(long gameId)
    {
        ...
        try { if (applied is not null) SetPower(applied); _running = Process.Start(new ProcessStartInfo(game.Executable) { WorkingDirectory = Path.GetDirectoryName(game.Executable), UseShellExecute = true }); if (_running is null) throw new InvalidOperationException("Could not start the game."); _running.EnableRaisingEvents = true; _running.Exited += (_, _) => Finish(id, previous, applied, "completed"); UpdateStatus(id, "running", null, false); } catch { Finish(id, previous, applied, "launch_failed"); throw; }
    }
